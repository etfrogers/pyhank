...
def test_bessel_zeros(order: int, n_zeros: int, engine):
    tolerance = 1e-5
    matlab_zeros = engine.bessel_zeros(1.0, float(order), float(n_zeros), float(tolerance), nargout=1)
...
def test_hankel_matrix(engine):
    matlab_to_python_mappings = {"N": "n_points", "p": "order", "alpha_N1": "alpha_n1", "V": "v_max"}
    r_max = 5e-3
    nr = 512
    h = HankelTransform(0, r_max, nr)
    hm = engine.hankel_matrix(0.0, r_max, float(nr), nargout=1)
    for key, matlab_value in hm.items():
        python_key = matlab_to_python_mappings.get(key, key)
        python_value = getattr(h, python_key)
        assert matlab_python_allclose(python_value, matlab_value), f"Hankel matrix key {key} doesn't match"
...
def example():
    ...
    z_max = 0.25  # Maximum propagation distance
    dz = z_max / (Nz - 1)
