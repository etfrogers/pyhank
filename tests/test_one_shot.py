...
def test_jinc(backend, radius: np.ndarray, a: float, order: int):
    f = generalised_jinc(radius, a, order)
    kr, actual_ht = backend.qdht(radius, f, order=order)
    v = kr / (2 * np.pi)
...
def test_top_hat(backend, radius: np.ndarray, a: float, order: int):
    f = generalised_top_hat(radius, a, order)
    kr, actual_ht = backend.qdht(radius, f, order)
    v = kr / (2 * np.pi)
...
def test_gaussian_2d(backend, axis: int, radius: np.ndarray):
    ...
    kr, actual_ht = backend.qdht(radius, f, axis=axis)
...
@pytest.mark.parametrize("axis", [0, 1])
def test_inverse_gaussian_2d(backend, axis: int):
    # Note the definition in Guizar-Sicairos varies by 2*pi in
    # both scaling of the argument (so use kr rather than v) and
    # scaling of the magnitude.
    kr = np.linspace(0, 200, 1024)
    ...
    r, actual_f = backend.iqdht(kr, ht, axis=axis)
...
def test_1_over_r2_plus_z2(backend, a: float):
    ...
    expected_ht = expected_ht[10:]  # type: ignore
...
@pytest.mark.parametrize("a", [1, 0.7, 0.1])
@pytest.mark.parametrize("order", orders)
def test_jinc_equivalence(backend, a: float, order: int, radius: np.ndarray):
    transformer = backend.HankelTransform(order=order, radial_grid=radius)
    f = generalised_jinc(radius, a, order)
    _, one_shot_ht = backend.qdht(radius, f, order=order)
...
@pytest.mark.parametrize("a", [1, 0.7, 0.1])
@pytest.mark.parametrize("order", orders)
def test_top_hat_equivalence(backend, a: float, order: int, radius: np.ndarray):
    transformer = backend.HankelTransform(order=order, radial_grid=radius)
    f = generalised_top_hat(radius, a, order)
    _, one_shot_ht = backend.qdht(radius, f, order=order)
...
@pytest.mark.parametrize("a", [2, 5, 10])
def test_gaussian_equivalence(backend, a: float, radius: np.ndarray):
    # Note the definition in Guizar-Sicairos varies by 2*pi in
    # both scaling of the argument (so use kr rather than v) and
    # scaling of the magnitude.
    transformer = backend.HankelTransform(order=0, radial_grid=radius)
    f = np.exp(-(a**2) * radius**2)
    _, one_shot_ht = backend.qdht(radius, f)
...
def test_1_over_r2_plus_z2_equivalence(backend, a: float):
    r = np.linspace(0, 50, 1024)
    f = 1 / (r**2 + a**2)
    transformer = backend.HankelTransform(order=0, radial_grid=r)
    f_transformer = 1 / (transformer.r**2 + a**2)
    assert np.allclose(transformer.to_transform_r(f), f_transformer, rtol=1e-2, atol=1e-6)

    kr, one_shot_ht = backend.qdht(r, f)
