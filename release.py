...
def main():  # pragma: no cover
    ...
    print(f"Current version is {current_version!s}")
    try:
        _, version = sys.argv
    except ValueError:
    ...
    if continue_response.lower().startswith("n"):
        return
    status = subprocess.run(["git", "status", "--porcelain"], capture_output=True, check=False)
    clean = status.stdout == b""
    do_stash = False
    if not clean:
        subprocess.run(["git", "status"], check=False)
        stash_response = input(
            "Working directory is not clean. Do you want to continue with current status, stash, or quit [c/S/q]?: "
        )
    ...
    if message == "":
        message = f"Release version {new_version!s}"
